    ...
    if(isset($_POST["grabar"]) && !empty($_POST)){
     ...
     //var_dump($_FILES["foto"]["tmp_name"]);
        if($_FILES["archivo"]["tmp_name"]!="") {
            echo 'nuevo';
            copy($_FILES["archivo"]["tmp_name"], "public/images/exercices/alfabeto/".$_FILES["archivo"]["name"]);
            $sPath = "public/images/exercices/alfabeto/".$_FILES["archivo"]["name"];
        } else {
            echo 'existe';
            $sPath = $_POST['oldUrl'];
        }
        ...
        if($sRes==TRUE) {
            header("Location: ad_new_alf.php?m=1");
        } else {
            header("Location: ad_new_alf.php?m=2");
        }
        die();
        /*
        //var_dump($aData);
